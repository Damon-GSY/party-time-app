...
test('native and preview share the warm paper palette semantics', () => {
  const nativeStyles = read('miniprogram/app.wxss').toLowerCase()
  const preview = read('preview.html').toLowerCase()

  for (const color of ['#f5f0e7', '#fffcf6', '#26382e', '#b83a2d', '#4e6851']) {
...
test('native and preview use the editorial two-column scheduler contract', () => {
  ...
  assert.doesNotMatch(nativeVote, /SCHEDULING|SLOT_SELECT|TOTAL_SELECTION|SELECTED|OPEN/)
...
test('home calendar expands from the current week into a complete month', () => {
  ...
  assert.match(nativeScript, /util\.generateMonthCalendar\(viewedMonth, today\)/)
  assert.match(nativeScript, /toggleCalendar\(\)/)
  assert.match(nativeScript, /showPreviousMonth\(\)/)
  assert.match(nativeScript, /showNextMonth\(\)/)
  assert.match(nativeScript, /showTodayMonth\(\)/)
  assert.match(nativeStyles, /\.calendar-month-grid \{[\s\S]*grid-template-columns: repeat\(7/)
  assert.match(nativeHome, /bindtap="showPreviousMonth"/)
  assert.match(nativeHome, /bindtap="showNextMonth"/)
  assert.match(nativeHome, /bindtap="showTodayMonth"/)
  ...
  assert.match(preview, /class="calendar-month-grid" id="home-month-grid" role="rowgroup"/)
  assert.match(preview, /class="calendar-month-row" role="row"/)
  ...
  assert.match(preview, /function changeHomeCalendarMonth\(offset\)/)
  assert.match(preview, /home-calendar-previous'\)\.addEventListener\('click'/)
  assert.match(preview, /home-calendar-next'\)\.addEventListener\('click'/)
  assert.match(preview, /home-calendar-today'\)\.addEventListener\('click', showCurrentHomeMonth\)/)
})
